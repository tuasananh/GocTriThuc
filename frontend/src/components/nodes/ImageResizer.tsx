...
export default function ImageResizer({
  ...
  const startPos = useRef<{ x: number; y: number; w: number; h: number; ratio: number } | null>(
    null,
  );
  ...
  return (
    <div
      className={`absolute inset-0 pointer-events-none border-2 border-blue-500 rounded-sm ${isResizing ? 'opacity-100' : 'opacity-0 hover:opacity-100 transition-opacity'}`}
    >
