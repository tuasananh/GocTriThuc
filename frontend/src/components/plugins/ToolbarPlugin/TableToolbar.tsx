...
import { $getNodeTriplet } from '@lexical/selection';
    ...
    return (
        <>
            ...
            <div className="flex items-center gap-1 bg-blue-50/20 dark:bg-blue-900/10 px-1 py-0.5 rounded-md border border-blue-100/50 dark:border-blue-900/20">
                <span className="text-[10px] font-bold uppercase tracking-wider text-blue-500/80 mx-1 select-none">Table</span>
                ...
                <DropDown
                    ...
                    title="Rows"
                >
                    <DropDownItem onClick={() => editor.update(() => $insertTableRowAtSelection(false))} className="item">
                        <span className="text">Insert Row Above</span>
                    </DropDownItem>
                    <DropDownItem onClick={() => editor.update(() => $insertTableRowAtSelection(true))} className="item">
                        <span className="text">Insert Row Below</span>
                    </DropDownItem>
                    <DropDownItem onClick={() => editor.update(() => $deleteTableRowAtSelection())} className="item">
                ...
                <DropDown
                    ...
                    title="Columns"
                >
                    <DropDownItem onClick={() => editor.update(() => $insertTableColumnAtSelection(false))} className="item">
                        <span className="text">Insert Column Left</span>
                    </DropDownItem>
                    <DropDownItem onClick={() => editor.update(() => $insertTableColumnAtSelection(true))} className="item">
                        <span className="text">Insert Column Right</span>
                    </DropDownItem>
                    <DropDownItem onClick={() => editor.update(() => $deleteTableColumnAtSelection())} className="item">
                ...
                >
                    <DropDownItem onClick={() => editor.update(() => {
                        const selection = $getSelection();
                        if ($isRangeSelection(selection) || $isTableSelection(selection)) {
                            const nodes = $isTableSelection(selection) ? selection.getNodes() : [selection.anchor.getNode()];
                            nodes.forEach(node => {
                                const cell = $getTableCellNodeFromLexicalNode(node);
