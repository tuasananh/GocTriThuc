...
**Done when**: Full test flow works end-to-end: start → answer → countdown → submit → view results. Countdown is crash-proof (server-calculated remaining time).
...
Scoring calculations automatically adapt to the number of active questions. If a question is deleted, points dynamically scale against active ones.
...
Initialize timer strictly using the server-calculated `remainingTime` to remain robust against page refreshes.

```tsx
useEffect(() => {
  Promise.all([
    api.get<TestDto>(`/api/tests/${testId}`),
    api.post<TestSessionDto & { remainingTime: number }>(`/api/tests/${testId}/sessions`),
