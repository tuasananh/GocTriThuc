...
Before any frontend starts on a feature, the backend must publish an **API contract** (URL, request body, response shape, error codes) as a GitHub Issue comment. The frontend unblocks themselves by writing MSW mocks matching the contract immediately.

---

## 🏆 Completed Foundation (Days 1–2)

- **Trung (BE Lead)**: Flyway sequence, initial user tables, SecurityConfig OAuth2 Google + GitHub configuration, and `/api/users/me` controllers.
- **Anh (BE Dev/PM)**: Roles tables database seeds (`student`, `teacher`, `admin`) and global session validations.
- **Vinh (FE Lead)**: Axios configuration, Vite proxy, MainLayout header drop navbar, and base routing scripts.
- **Sâm (FE Dev 1)**: DTO types, Loader components, and GuestRoute guards.
- **Tuấn (FE Dev 2)**: LoginPage forms, Dashboard pages, and global styling tokens.
